"""Generate FineReport custom-function Java for ``ChineseConvertUtil``.

The Housing Authority tenancy agreement (NDMS-TN-0028) renders dates and money
amounts as Chinese text via a Java helper that FineReport cannot run natively.
``graft.translate.jasper_to_finereport`` rewrites ``ChineseConvertUtil.foo(...)``
calls to bare ``foo(...)`` FineReport functions; this module emits the matching
Java so the user can drop the compiled classes into
``%FR_HOME%/webapps/webroot/WEB-INF/classes/`` and register each function.

The conversion logic mirrors ``graft.translate.chinese_convert`` (the Python
reference, which is tested against the rendered agreement).
"""
...
# FineReport function name -> the ChineseConvertUtil static method it wraps.
CUSTOM_FUNCTIONS: dict[str, str] = {
    "dateToChineseYear": "dateToChineseYear",
    "dateToChineseMonth": "dateToChineseMonth",
    "dateToChineseDay": "dateToChineseDay",
...
_FUNCTION_TEMPLATE = """package com.fr.function;

import com.fr.script.AbstractFunction;

/** FineReport custom function: {fn}({arg_desc}). */
public class {fn} extends AbstractFunction {{
    ...
    public Object run(Object[] args) {{
        ...
        return ChineseConvertUtil.{method}(args[0]);
    }}
}}
"""

_ARG_DESC = {
    "dateToChineseYear": "a date",
    "dateToChineseMonth": "a date",
    "dateToChineseDay": "a date",
    "numberToChinese": "an integer",
    "decimalToChinese": "a money amount",
}


def write_custom_functions(out_dir: Path) -> list[Path]:
    """Write the ChineseConvertUtil helper and one class per custom function.
    ...
    helper = out / "ChineseConvertUtil.java"
    helper.write_text(_HELPER_JAVA, encoding="utf-8")
    written.append(helper)

    for fn, method in CUSTOM_FUNCTIONS.items():
        path = out / f"{fn}.java"
        path.write_text(
